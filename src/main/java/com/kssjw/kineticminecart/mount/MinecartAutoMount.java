...
public final class MinecartAutoMount {
    ...
    public static void maybeAutoMount(AbstractMinecart minecart) {
        ...
        // 仅服务器端
        if (minecart.getLevel().isClientSide()) return;
        if (!(minecart.getLevel() instanceof ServerLevel)) return;
        ...
        BlockState state = minecart.getLevel().getBlockState(pos);
        ...
        List<Entity> nearby = minecart.getLevel().getEntities(minecart, box, canAutoMountFilter(minecart));
    ...
    private static Predicate<Entity> canAutoMountFilter(AbstractMinecart minecart) {
        return entity -> {
            ...
            // 排除情况
            if (entity.getLevel().isClientSide()   // 排除客户端
